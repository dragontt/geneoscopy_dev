...
python ${DIR_SCRIPTS}/split_cv_folds.py -i ${DIR_DATA}/training/chipdata.txt -v ${DIR_DATA}/training/valid_chips.txt -f 20 -g $GROUP -o ${DIR_DATA}/training/cv_folds
for i in {1..20}; do
...
for NUM_TOP_GENES in "${NUM_TOP_GENES_LIST[@]}"; do
	...
	echo "#################################"
	echo ""
	python ${DIR_SCRIPTS}/summarize_cv_de_genes.py -i ${DIR_DATA}/training/cv_folds -f 20 -t $NUM_TOP_GENES -o ${DIR_DATA}/training/top_de_genes.txt
