...
def main(argv):
	...
	output_expr = np.vstack((np.append(['sample','label'], top_genes)[np.newaxis],
		np.hstack((np.array(sample_ids)[np.newaxis].T, np.array(labels)[np.newaxis].T,
			input_expr[top_indx,:].T))))
